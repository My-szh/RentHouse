...
public class LeaseAgreementController {
    ...
    public Result<List<AgreementItemVo>> listItem() {
        List<AgreementItemVo> list = service.listItem(LoginUserHolder.getLoginUser().getUsername());
    ...
    public Result<List<PaymentType>> PaymentTypeList(@RequestParam Long id) {
        List<PaymentType> list = service.listByRoomId(id);
        return Result.ok(list);
    }

    @GetMapping("LeaseTermList")
    @Operation(summary = "根据房间id获取可选获取租期列表")
    public Result<List<LeaseTerm>> LeaseTermList(@RequestParam Long id) {
        List<LeaseTerm> list = service.getLeaseTerms(id);
        return Result.ok(list);
    }
